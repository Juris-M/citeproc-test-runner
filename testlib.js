...
if (fs.existsSync(path.join(__dirname, "..", "citeproc_commonjs.js"))) {
    var CSL = require(path.join(__dirname, "..", "citeproc_commonjs.js"));
} else {
    var CSL = require("citeproc");
}

if (fs.existsSync(path.join(__dirname, config.path.locale))
    && fs.readdirSync(path.join(__dirname, config.path.locale)).length) {

    var localePath = path.join(__dirname, config.path.locale);
} else {
    var localePath = require("citeproc-locales");
    console.log("trying with "+localePath);
}

if (fs.existsSync(path.join(__dirname, config.path.modules))
    && fs.readdirSync(path.join(__dirname, config.path.modules)).length) {
    var modulePath = path.join(__dirname, config.path.modules);
} else {
    var modulePath = require("citeproc-juris-modules");
}
...
Sys.prototype.retrieveLocale = function(lang){
    var ret = null;
    try {
        ret = fs.readFileSync(path.join(localePath, "locales-"+lang+".xml")).toString();
        ret = ret.replace(/\s*<\?[^>]*\?>\s*\n/g, "");
    } catch (e) {
        ret = false;
    }
...
Sys.prototype.retrieveStyleModule = function(jurisdiction, preference) {
    ...
    id = id.join("-");
    try {
        ret = fs.readFileSync(path.join(modulePath, "juris-" + id + ".csl")).toString();
    } catch (e) {}
    return ret;
};
