...
if (fs.existsSync(config.path.src && path.join(config.path.src, "..", "citeproc_commonjs.js"))) {
    try {
        ...
        console.log("Warning: syntax error in processor code");
    }
} else {
...
Sys.prototype.preloadAbbreviationSets = function() {
    if (!config.path.jurisAbbrevPath) return;
    for (var itemID in this._cache) {
        ...
        var jurisAbbrevFilePath = path.join(config.path.jurisAbbrevPath, "auto-" + country + ".json");
